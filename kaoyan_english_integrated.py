...
# ============================================================================
# 作文模板报告模块：由 ENGlishtext.py 合并而来
# 说明：
#   - 这一部分保留了原来“考研英语二作文模板生成器”的核心流程。
#   - 与客观题复盘模块共用 GEMINI_API_KEY、MODEL_NAME、API_BASE_URL、OUTPUT_DIR。
#   - 不再硬编码 API Key，不再写死 D 盘路径。
#   - 输出 Markdown 到 OUTPUT_DIR，检查点写入 CHECKPOINT_DIR。
# ============================================================================

WRITING_CHECKPOINT_PATH = CHECKPOINT_DIR / "KY_English2_Advanced_ckpt.json"
WRITING_MD_PATH = OUTPUT_DIR / "考研英语二作文_高级模板报告_V3.md"
...
- 2023小作文：给David建议艺术展还是机器人展的邮件

提示：如果你想让 2010-2025 的主题总结更准确，请在本字符串中补全历年真题主题。
不要要求模型凭空“准确总结”你没有提供的数据。
"""


def call_gemini_for_writing(prompt_text: str, step: str = ""):
    """作文模板模块专用的 Gemini 调用。

    客观题复盘的 call_gemini 默认会携带 PDF；作文模板不需要 PDF，
    因此这里单独封装一个纯文本调用，避免互相干扰。
    """
    if not API_KEY:
        raise RuntimeError("请先设置环境变量 GEMINI_API_KEY，或在 .env 中填写。")
    ...
    print("🔥 正在让 Gemini 生成 %s（预计 15~40 秒）..." % step)
    resp = requests.post(url, json=payload, timeout=180)
    if resp.status_code == 200:
        data = resp.json()
        candidates = data.get("candidates", [])
        if not candidates:
            raise RuntimeError("Gemini 返回为空：没有 candidates")
        content = candidates[0].get("content", {})
        parts_out = content.get("parts", [])
        if not parts_out or "text" not in parts_out[0]:
            raise RuntimeError("Gemini 返回为空：没有 text")
        text = parts_out[0]["text"]
        print("✅ %s 完成！" % step)
        return text

    print("❌ %s 失败 HTTP %s" % (step, resp.status_code))
    print(resp.text[:500])
    raise RuntimeError("作文模板模块 API 调用失败")


def build_writing_prompt_topics(topics_raw: str) -> str:
    return (
        "你是考研英语专家。\n"
        + topics_raw
        + "\n请基于我提供的信息，输出考研英语（二）大作文和小作文主题总结。"
          "要求：\n"
          "1. 不要编造我没有提供的年份细节；\n"
          "2. 已知信息必须准确；\n"
          "3. 不确定的年份请标注“待补充”；\n"
...
def build_writing_prompt_templates(topics_text: str) -> str:
    return f"""
你是考研英语（二）作文提分老师。
...
def build_writing_prompt_big_example(templates_text: str) -> str:
    return f"""
请使用下面模板中的饼图写法，为 2023 考研英语（二）大作文写一篇完整范文。

题目背景：消费者选择餐厅时关注的因素饼状图，包含特色、服务、环境、价格等因素。
...
def build_writing_prompt_small_example() -> str:
    return """
为 2023 考研英语（二）小作文写一篇完整建议信范文。

题目背景：给 David 写邮件，建议他去看艺术展还是机器人展。
...
def build_writing_prompt_optimize(topics: str, templates: str, example_big: str, example_small: str) -> str:
    return f"""
你是考研英语（二）作文模板教练。请综合以下所有内容，做一次最终优化。
...
def assemble_writing_report(results: dict) -> str:
    return f"""# 考研英语二作文高级模板报告 V3（记忆薄弱版）
...
## 三、2023 大作文实操示范（餐厅选择因素饼图）

{results.get('example_big', '')}

---

## 四、2023 小作文实操示范（建议信）

{results.get('example_small', '')}
...
> 说明：本报告由 Gemini 生成，建议结合真题答案和老师讲解复核后再背诵。
"""


def run_writing_report():
    """运行作文模板报告生成流程。"""
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)

    print("=" * 100)
    print("🚀 考研英语二作文模板报告生成器 V3（整合版）")
    print("   专为记忆薄弱者设计 | 已移除硬编码密钥 | GitHub 安全版")
    print("=" * 100)

    if WRITING_CHECKPOINT_PATH.exists():
        ckpt = json.loads(WRITING_CHECKPOINT_PATH.read_text(encoding="utf-8"))
    else:
        ckpt = {"results": {}}

    key = "advanced_english2_v3_2026_integrated"
    if key in ckpt.get("results", {}):
        print("✅ 检查点已存在，直接写出报告")
        WRITING_MD_PATH.write_text(ckpt["results"][key], encoding="utf-8")
        print("报告已保存 → " + str(WRITING_MD_PATH))
        return
    ...
    results = {}
    with tqdm(total=14, desc="🟢 作文进度", colour="green", unit="步", ncols=100, mininterval=0.3) as pbar:
        pbar.set_description("1/14 " + stages[0])
        print("📂 正在加载检查点...")
        results["topics_raw"] = WRITING_PAST_TOPICS
        pbar.update(2)

        pbar.set_description("3/14 " + stages[2])
        prompt1 = build_writing_prompt_topics(results["topics_raw"])
        results["topics"] = call_gemini_for_writing(prompt1, "4/14 " + stages[3])
        pbar.update(2)

        pbar.set_description("5/14 " + stages[4])
        prompt2 = build_writing_prompt_templates(results["topics"])
        results["templates"] = call_gemini_for_writing(prompt2, "5/14 " + stages[4])
        pbar.update(1)

        pbar.set_description("6/14 " + stages[5])
        prompt3 = build_writing_prompt_big_example(results["templates"])
        results["example_big"] = call_gemini_for_writing(prompt3, "7/14 " + stages[6])
        pbar.update(2)

        pbar.set_description("8/14 " + stages[7])
        prompt4 = build_writing_prompt_small_example()
        results["example_small"] = call_gemini_for_writing(prompt4, "9/14 " + stages[8])
        pbar.update(2)

        pbar.set_description("10/14 " + stages[9])
        prompt5 = build_writing_prompt_optimize(
            results["topics"],
            results["templates"],
            results["example_big"],
            results["example_small"],
        )
        results["optimized"] = call_gemini_for_writing(prompt5, "10/14 " + stages[9])
        pbar.update(3)

        pbar.set_description("13/14 " + stages[12])
        print("💾 正在保存检查点和报告...")
        final_report = assemble_writing_report(results)
        pbar.update(2)

    ckpt.setdefault("results", {})[key] = final_report
    WRITING_CHECKPOINT_PATH.write_text(json.dumps(ckpt, ensure_ascii=False, indent=2), encoding="utf-8")
    WRITING_MD_PATH.write_text(final_report, encoding="utf-8")

    print("\n🎉 作文报告生成完成 → " + str(WRITING_MD_PATH))
    print("打开 Markdown 文件即可打印背诵。")
...
def print_runtime_check():
    ...
    print("GEMINI_API_KEY:", "已设置" if API_KEY else "未设置")
    print("GCS_URI:", GCS_URI if GCS_URI else "未设置")
    print("客观题输出:", MD_PATH)
    print("作文输出:", WRITING_MD_PATH)
    print("=" * 80)
    print("可用命令：")
    print("  python kaoyan_english_integrated.py check")
    print("  python kaoyan_english_integrated.py writing")
    print("  python kaoyan_english_integrated.py objective")
    print("  python kaoyan_english_integrated.py all")
    print("=" * 80)


def main():
    parser = argparse.ArgumentParser(
        description="考研英语（二）AI 复盘整合脚本：客观题复盘 + 作文模板报告。"
    )
    parser.add_argument(
        "mode",
        nargs="?",
        default="check",
        choices=["check", "writing", "objective", "all"],
        help="运行模式：check=环境检查；writing=作文模板报告；objective=2023客观题复盘；all=先作文后客观题。默认 check。",
    )
